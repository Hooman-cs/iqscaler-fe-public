...
const ResultScreen = () => {
  ...
  // --- NEW EFFECT: Fetch Price/Order on Load ---
  useEffect(() => {
    const fetchOrderDetails = async () => {
      // Only fetch if we have result details, it's not purchased, and we haven't fetched price yet
      if (resultDetails && !resultDetails.certificatePurchased && !displayPrice) {
        try {
          const { data } = await api.post('/payments/create-order', { resultId });
          setDisplayPrice(data.amount / 100);
          setOrderDetails(data); // Store the order details for the button click later
        } catch (err) {
          console.error("Failed to fetch price on load:", err);
          // We don't block the UI here, we just won't show the price until they click
        }
      }
    };

    fetchOrderDetails();
  }, [resultDetails, displayPrice, resultId]);
...
// --- SHARING LOGIC ---
  ...
  const handlePurchaseCertificate = async () => {
    ...
    try {
      ...
      // 1. If we haven't fetched the order details on mount, fetch them now
      if (!data) {
      ...
      const options = {
        ...
        theme: { color: '#3b82f6' }, // Tailwind blue-500 equivalent
      };
